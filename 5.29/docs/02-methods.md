...
行文遵循 `重要原则.txt`：书面语、正式语气；以步骤与机制为叙述单位，不罗列仓库内的程序文件；若必须引用代码中的标识符（边类型、语义标签等），均附简要含义。
...
| **第 3 部分** | stage4 | 未并入直墙的碎线区域具有何种巷道语义 |
...
第 1 部分回答：从导出的直线与圆弧图元出发，如何得到**仅含直线的归一化几何**、**端点邻接图**，以及**直墙与碎线的平行逻辑图**。本部分不做巷道候选、不做主巷/横档/腔室语义判定。
...
- **只记录，不改坐标**。方折信息供审计与后续语义参考，不参与本部分的圆弧裁端。
...
| **残差碎线** | 未并入直墙的短段 | 保留原句柄；进入平行逻辑图的碎线节点；语义在第 3 部分标注 |
...
**合成连接器**（与横档语义无关）：若某一平行残差分量在轴向两端均已挂接既有走廊拓扑，可沿平行方向补写一段中心线以闭合几何连续性。该操作只修复断裂，**不是**横档识别，也**不是**腔室判别。

**输出**：修补后的中心线逻辑图。

---

## 第 3 部分：残差区域语义（stage4）
...
分类**按碎线逐条**进行，与残差簇分组无关；四条规则按固定顺序执行，先命中先标注：
...
每条碎线附带置信度、分类来源及所属残差簇编号，供后续主巷网络构建过滤使用。
...
| 可能巷壁语义（形状重标） | 第 3 部分 | 标注与边界墙平行的碎线形状 |
| 合成连接器 | 第 2 部分后半 | 补写中心线几何连续性 |
| 辅巷道语义 | 第 3 部分 | 标注横档形平行碎线对 |

### 3.4 输出

**拓扑输出**：语义残差图——残差图之上为每个碎线节点写入语义标签与审计字段。
...
第 3 部分
  语义残差图 ──→ 更后阶段（主巷连通时排除腔室、保留横档等，未在本文展开）
```
