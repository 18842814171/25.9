...
def resolve_matching_layers(file_path, patterns: Iterable[str] | None) -> list[str]:
    """列出图纸中命中子串模式的图层名（排序去重）。"""
    if not patterns:
        return []
    pats = [p.strip() for p in patterns if p and str(p).strip()]
    if not pats:
    ...
    for layer in doc.layers:
        ...
        if layer_matches_substrings(name, pats):
            matched.add(name)
    return sorted(matched)


def filter_msp(file_path, desired_types=None, desired_layers=None):
    """
    ...
    for entity in msp:
        ...
        if layer_patterns and not layer_matches_substrings(layer, layer_patterns):
            continue
