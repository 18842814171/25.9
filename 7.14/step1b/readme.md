...
本目录将几何结构图与第一阶段甲产出的最终簇图挂接，得到带标注的结构图。结构图路径须在命令行用参数显式指定，代码内不写跨目录默认路径。阈值只在 [`config.py`](config.py) 中配置。总体进度见 [`docs/01-architecture.md`](../docs/01-architecture.md)。
...
| 最终簇图 | 默认读取 `step1a/output/{图号}-final_cluster.pkl`；可用参数覆盖 |
| 产物 | `step1b/output/{图号}-structure_graph_with_texts` 的二进制图、可读摘要与核对图 |

按《重要原则》第 7 条，完整步骤宜只输入一个逻辑拓扑图。本步当前同时读入结构图与最终簇图，与该条尚有差距。

## `build_fusion.py`（融合核心，非 CLI）

本文件是挂接逻辑库，**不能**直接 `python step1b/build_fusion.py`。命令行入口是 `0_structure_graph_with_texts.py`，它读入结构图与最终簇图后调用本库，再写出产物。

| 符号 | 作用 |
|------|------|
| `build_structure_graph_with_texts(structure, clusters, cfg)` | 深拷贝结构图；把簇节点与未入簇文字挂到最近中心线；写回 `attach_threshold` / `attach_summary` |
| `collect_isolated_text_nodes(clusters, cfg)` | 收集最终簇图中未入簇的 `TEXT`/`MTEXT`，一律当作巷道名称候选 |

挂接行为要点：

- 仅向配置中的中心线角色挂接（默认 `corridor`、`auxiliary`）。
- 挂接距离阈值由候选点到最近中心线距离的统计量推断（分位数、离群上界、回退倍数见 `config.py`）。
- 簇与成员以 `member` 边相连；挂到中心线的边为 `on-centerline`。
...
| `--clusters-pkl` | 可选；最终簇图路径。省略时读 `step1a/output/{stem}-final_cluster.pkl` |
| `--step1a-output-dir` | 可选；未指定 `--clusters-pkl` 时，在该目录下找最终簇图 |
...
本步不重新分类测点与钻孔，只做空间挂接；未入簇文字一律视为巷道名称并尝试挂接。挂接距离由候选点到最近中心线距离的统计量推断（分位数、离群上界、回退倍数等均在顶层配置）。
