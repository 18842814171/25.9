...
def extract_entity_metadata(entity: ezdxf.entities.DXFEntity, recurse_blocks: bool = True) -> List[Dict[str, Any]]:
    ...
    if entity_type == 'DIMENSION':
        ...
        metadata['scale'] = entity.dxf.pattern_scale
        metadata['angle'] = entity.dxf.pattern_angle
        try:
        ...
        metadata['closed'] = entity.is_closed
...
def layer_matches_substrings(layer: str, patterns: Iterable[str] | None) -> bool:
    """图层名是否命中任一子串模式（空模式视为不限制）。"""
    if not patterns:
        return True
    pats = [p.strip() for p in patterns if p and str(p).strip()]
    if not pats:
        return True
    return any(p in layer for p in pats)


def layer_excluded_by_keywords(
    layer: str, keywords: Iterable[str] | None
) -> bool:
    """图层名是否命中任一排除关键词（子串；空列表视为不排除）。"""
    if not keywords:
        return False
    keys = [k.strip() for k in keywords if k and str(k).strip()]
    if not keys:
        return False
...
def layer_allowed(
    ...
    """先按 layers 子串命中，再去掉含排除关键词的图层。"""
    if include_patterns is not None:
        pats = [p.strip() for p in include_patterns if p and str(p).strip()]
        if pats and not layer_matches_substrings(layer, pats):
            return False
    if layer_excluded_by_keywords(layer, exclude_keywords):
        return False
    return True


def resolve_matching_layers(
    file_path,
    patterns: Iterable[str] | None,
    exclude_keywords: Iterable[str] | None = None,
) -> list[str]:
    """列出图纸中命中 layers 且未被排除关键词去掉的图层名（排序去重）。"""
    if not patterns:
        return []
    pats = [p.strip() for p in patterns if p and str(p).strip()]
    if not pats:
    ...
    for layer in doc.layers:
        name = layer.dxf.name.strip()
        if layer_allowed(name, pats, exclude_keywords):
            matched.add(name)
...
def filter_msp(
    ...
    """
    Filter entities in modelspace by type and layer.
    desired_layers 为子串模式列表：图层名包含任一子串即命中（非全等）。
    exclude_layer_keywords：在已命中图层中，名称含任一关键词则排除。
    """
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()
    ...
    # Normalize filters
    if desired_types:
        desired_types = set(desired_types)

    layer_patterns = None
    if desired_layers:
        layer_patterns = [l.strip() for l in desired_layers if l and str(l).strip()]

    exclude_keywords = None
    if exclude_layer_keywords:
        exclude_keywords = [
            k.strip() for k in exclude_layer_keywords if k and str(k).strip()
        ]
    ...
    for entity in msp:
        ...
        if desired_types and entity_type not in desired_types:
            continue

        layer = entity.dxf.layer.strip() if hasattr(entity.dxf, 'layer') else '0'

        if not layer_allowed(layer, layer_patterns, exclude_keywords):
            continue
...
def extract_entities_in_window_by_layer(dxf_path: str, window_corners: Optional[tuple] = None) -> LayerData:
    """
    从 DXF 文件的 Modelspace 中提取窗口内的实体及其元数据，按图层和块分组。
    
    参数:
        dxf_path: DXF 文件路径。
        window_corners: ((x1,y1), (x2,y2)) 窗口对角点，可选。
    ...
    # 修复：根据 window_corners 是否为 None 来决定是否使用窗口过滤
    if window_corners is not None:
        ...
        # 当 window_corners 为 None 时，不使用窗口过滤，获取所有实体
...
# -------------------- Usage --------------------
def exec():
    dxf_file_path = r"D:\大创\25.9\图纸\附图2巷道布置图2.dxf"
    ...
    output_file=r"附图2实体.txt"
    original_stdout = sys.stdout
    with open(output_file, 'w', encoding='utf-8') as f:
        sys.stdout = f
        
        # Extract from window
        all_layer_data = extract_entities_in_window_by_layer(dxf_file_path,window_corners=None)
