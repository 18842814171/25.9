...
LayerData = Dict[str, Dict[str, List[Dict[str, Any]]]]  # {layer: {group_type: [entities]} }

from entities_filter import filter_msp, list_entity_from_msp
from text_clean import plain_mtext
...
def json_indep_hatch(entity,attributes):
    ...
    # 边界折点，供设施核对图绘制（与 5.29 字段并存，不改既有键）
    ...
    try:
        for path in entity.paths:
            ...
            if verts is not None:
                for v in verts:
                    path_points.append([float(v[0]), float(v[1])])
                continue
            ...
            for edge in edges:
                ...
                if end is not None:
                    path_points.append([float(end[0]), float(end[1])])
    except Exception:
        path_points = []
    attributes["path_points"] = path_points
    return attributes            
...
def json_indep_lwpolyline(entity, attributes):
    # Use 'xyseb' format to get: x, y, start_width, end_width, bulge
    # This captures the full geometry of every segment
    attributes['points'] = [list(p) for p in entity.get_points(format='xyseb')]
    
    # Structural properties
    attributes['closed'] = entity.closed
    attributes['elevation'] = entity.dxf.elevation
    attributes['const_width'] = entity.dxf.const_width
    
    # 3D Orientation (Crucial for non-XY plane polylines)
    attributes['extrusion'] = list(entity.dxf.extrusion)
    
    # Visual Styling
    attributes['layer'] = entity.dxf.layer
    attributes['color'] = entity.dxf.color
    attributes['linetype'] = entity.dxf.linetype
    attributes['lineweight'] = entity.dxf.lineweight
    
    # Linetype generation (True if pattern flows across vertices)
    attributes['plinegen'] = bool(entity.dxf.flags & 128)
...
def json_indep_mtext(entity,attributes):
    # 导出可见纯文本，避免把 \f/\H 等 MTEXT 格式码带进下游匹配
    attributes['text'] = plain_mtext(entity)
    attributes['insert_point'] = list(entity.dxf.insert.xyz)
