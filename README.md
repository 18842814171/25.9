...
根目录批处理脚本只负责「多样本编排、产物收集与指标汇总」，不合并算法步骤本身。巷道几何、标注关联、设施识别等具体步骤仍分别位于 `5.29/` 与 `7.14/` 的独立脚本中，由流水线按既定顺序逐一调用。

整图与局部图严格分开：

| 入口 | 对象 | 整图专用步骤 |
|------|------|--------------|
| `run_full_drawing` | 仅整图 | 标注图例模板与识别规则（不负责 DXF 提取） |
| `run_stats` | 仅局部图 | 不执行上述步骤；引用整图已写出的识别规则 |
...
若整图产物中缺少识别规则，局部批处理将直接退出并提示先执行 `run_full_drawing`。不得在局部图上重复抽取图例或重新标定识别规则。
...
```text
python batch_export_test_input.py --src 2026.1-1/2026.1-1.dxf --cfg 2026.1-1/config.json --with-legend

python batch_export_test_input.py --cfg 2026.1-1/config.json
...
对指定整图跑通 `5.29` 与 `7.14` 全部计算步骤（含标注图例模板、识别规则标定）；写出 `{整图图号}_output` 与阶段统计。**不调用导出脚本**；须事先完成步骤 1 的整图导出（含图例）。

```text
.\run_full_drawing.bat --src 2026.1-1/2026.1-1.dxf
...
扫描 `test_input/*.dxf`，自动跳过整图 DXF；对每张局部图调用几何与标注步骤，**不**运行标注图例抽取、识别规则标定；通过 `--rules-json` 引用整图产物。**不调用导出脚本**；局部图 JSON 须事先导出。

须显式传入 `--src <整图路径>`；省略则拒绝运行。

```text
.\run_stats.bat --src 2026.1-1/2026.1-1
.\run_stats.bat --src 2026.1-2\2026.1-2
run_stats.bat --src 2026.1-1 --output-root D:\自定义输出根目录

```

说明：`--src` 为整图路径（与 `run_full_drawing` 相同约定）；图号取其 basename，用于定位识别规则。须已存在对应产物，否则拒绝运行。向各 Python 步骤仅传入 `--stem=图名`。
...
- 根目录脚本不替代步骤脚本：圆角规范化、方折处理、标注组构建、设施关联等仍分文件、分步骤运行。
