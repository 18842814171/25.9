#!/usr/bin/env bash
# 局部图批处理：不抽取图例、不定制识别规则；引用整图已产出的规则与设施模板。
# 整图须先完成：./run_full_drawing.sh --src <path>
#
# 用法：
...
#
# --src：整图路径（相对代码根目录）；图号取 basename。传给 Python 仅为 --stem=图名。
...
usage() {
  echo "Usage: ./run_stats.sh --src <path> [--output-root DIR]"
  echo "Prerequisite: ./run_full_drawing.sh --src <path>"
  echo "Example: ./run_stats.sh --src 2026.1-1"
  echo "Example: ./run_stats.sh --src 2026.1-2/2026.1-2"
}
...
REL="${REL%.dxf}"
REL="${REL%.DXF}"
FULL_STEM="$(basename "$REL")"

if [[ -z "$OUTPUT_ROOT" ]]; then
  FULL_OUT="${CODE_ROOT}/${FULL_STEM}_output"
else
  FULL_OUT="${OUTPUT_ROOT}/${FULL_STEM}_output"
fi
...
run_one() {
  ...
  local S529_2="${OUT}/529-stage2"
  local S2A_RAW="${S529_2}/step2A/raw"
  local S2A_OUT="${S529_2}/step2A/output"
  local S2B_OUT="${S529_2}/step2B/output"
  local S529_3="${OUT}/529-stage3"
  local S3A_OUT="${S529_3}/step3A/output"
  local S3B_OUT="${S529_3}/step3B/output"
  local S529_4="${OUT}/529-stage4"
  local S714_1="${OUT}/714-stage1"
  local S714_2="${OUT}/714-stage2"

  local GEO_JSON="${TEST_INPUT}/${STEM}-巷道.json"
  local TEXT_JSON="${TEST_INPUT}/${STEM}-文字.json"
  local FACILITY_JSON="${TEST_INPUT}/${STEM}-设施.json"
  local STRUCTURE_PKL="${S529_4}/${STEM}_structure_graph.pkl"
  local TEXTS_PKL="${S714_1}/${STEM}-structure_graph_with_texts.pkl"
...
shopt -s nullglob
mapfile -t DXFS < <(printf '%s\n' "${TEST_INPUT}"/*.dxf "${TEST_INPUT}"/*.DXF 2>/dev/null | awk 'NF && !seen[tolower($0)]++')
if [[ ${#DXFS[@]} -eq 0 ]]; then
  echo "No DXF found under ${TEST_INPUT}"
  exit 1
fi
