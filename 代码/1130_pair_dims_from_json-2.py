...
def find_nearest_line_for_slope(dim: Dict[str, Any], lines: List[Dict[str, Any]]) -> tuple:
    """Find nearest line for slope dim."""
    ...
    for line in lines:
        ...
        if dist < min_dist and dist < 200.0:  # Tol for "near"
            min_dist = dist
            best_line = line
    
    if best_line:
        slope_val = parse_dim_value(dim)  # Always use parsed value
        return best_line, slope_val
    return None, 0.0

def find_best_line_for_length(dim: Dict[str, Any], lines: List[Dict[str, Any]], used_lines: set) -> tuple:
    """Find best line for length dim (allow partial multiples)."""
    attrs = dim["attributes"]
...
def pair_dimensions_with_entities_combined(all_entities: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for point_key in ['defpoint', 'defpoint2', 'defpoint3', 'defpoint4', 'defpoint5', 'defpoint6']:
            if point_key in attrs:
                ...
                if point == [0.0, 0.0, 0.0]:
                    continue
                entity = find_entity_at_point(point, lines, tolerance=10.0)
                if entity:
    ...
    # Fallback for unpaired lengths with partial
    used_lines = set()
...
output_file = "info/1026_paired_combined.json"
...
Processing 11 dims and 15 lines...

Made 12 pairings
Enhanced 36C: partial_lengths=None, slope=68.0
Enhanced 36D: partial_lengths=[300.0, 300.0, 500.0, 500.0, 400.0, 500.0], slope=68.0  (multi-partial)
Enhanced 36F: partial_lengths=None, slope=68.0
Enhanced 377: partial_lengths=None, slope=68.0
Enhanced 378: partial_lengths=300.0, slope=68.0

Final Results:
Lines with length dimensions: 2
Lines with slope dimensions: 5
Unused dimensions: -1
Enhanced entities saved to: info/1026_paired_combined.json

Enhanced lines summary:
Line 36C: partial_lengths=None, slope=68.0
Line 36D: partial_lengths=[300.0, 300.0, 500.0, 500.0, 400.0, 500.0], slope=68.0  (multi-partial)
