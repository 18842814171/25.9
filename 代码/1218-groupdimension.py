...
def snap_dimension(dim, h_groups, v_groups):
    ...
    if kind == "vertical":
        # Vertical dimension measures Y distance (row)
        # p2 and p3 have different Y coordinates
        g1 = snap_to_nearest(p2[1], h_groups, "y")
        g2 = snap_to_nearest(p3[1], h_groups, "y")
        ...
        # Horizontal dimension measures X distance (column)
        # p2 and p3 have different X coordinates
        g1 = snap_to_nearest(p2[0], v_groups, "x")
        g2 = snap_to_nearest(p3[0], v_groups, "x")
...
# Load data
dimensions = load_json("info\\1202new\\1202_export-topview.json")

# Load the group file (contains both horizontal and vertical)
group_data = load_json("1218-grouptop.json")
...
def save_grid_points(grid, output_file="grid_points.json"):
...
def save_detailed_report(row_constraints, col_constraints, row_coords, col_coords, 
                         grid, output_file="detailed_report.txt"):
...
if row_y and col_x:
    ...
    save_corrected_coordinates(row_y, col_x, "corrected_coordinates.json")
    
    # 2. Save grid points
    save_grid_points(grid, "grid_points.json")
    
    # 3. Save detailed text report
    save_detailed_report(row_constraints, col_constraints, row_y, col_x, grid, "detailed_report.txt")
